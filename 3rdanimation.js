...
function draw() {
  ...
  if (Y !== null) {
    Y.disp();
    fill(Y.color); // Noir pour le texte
    textSize(10);
    textAlign(CENTER, CENTER);
    text("Y", Y.initial.x, Y.initial.y);
  }
  ...
  if (Z !== null) {
    ...
    textSize(10);
    textAlign(CENTER, CENTER);
    text("Z", Z.initial.x, Z.initial.y);
  }

  if (X !== null) {
    X.disp();
    fill(X.color); // Noir pour le texte
    textSize(10);
    textAlign(CENTER, CENTER);
    text("X", X.initial.x, X.initial.y);
  }
...
function startAnimation() {
  ...
  retractButton.mousePressed(() => {
    ...
    X = retractTether(tether, (w = 3), (c = "blue"));
...
function nextEvent(event, anchorIdx, startIdx, endIdx, points) {
  while (buttons.length > 0) {
    let lastButton = buttons.pop();
    lastButton.remove(); // Removes the button from the DOM
  }
  ...
  nextTerminal.mousePressed(() => {
    ...
    console.log("yIdx: ", JSON.stringify(pathIdx.concat(ySliceIdx)));
    console.log("zIdx: ", JSON.stringify(pathIdx.concat(zSliceIdx)));
    ...
    Z.color = "green";
    event++;
    nextEvent(event, anchorIdx, startIdx, endIdx, points);
  });
  let retractyButton = createButton("Retract Y cable");
  retractyButton.position(30, buttons.length * 30 + 85);
  buttons.push(retractyButton); // Add to array to keep track
  retractyButton.mousePressed(() => {
    try {
      console.log("retracting Y");
      Y = retractTether(Y, Y.width, Y.color);
      console.log("retracted length: ", Y.getL());
    } catch (e) {
      throw console.error("error retracting Y: ", e);
    }
  });

  let retractzButton = createButton("Retract Z cable");
  retractzButton.position(30, buttons.length * 30 + 85);
  buttons.push(retractzButton); // Add to array to keep track
  retractzButton.mousePressed(() => {
    try {
      console.log("retracting Z");
      Z = retractTether(Z, Z.width, Z.color);
      console.log("retracted length: ", Z.getL());
    } catch (e) {
      throw console.error("error retracting Z: ", e);
    }
  });
}
...
function retractTether(T, width = 1, color = "black") {
  ...
  let crossed_triangles = findTrianglesCrossed(T, triangulation);

  crossed_triangles = removeRedundantTriangles(crossed_triangles);

  sleeve = liftPolygon(T, crossed_triangles);
...
function removeRedundantTriangles(crossedTriangles) {
  console.log("Entrée initiale :", crossedTriangles);

  // Copier la liste des triangles pour éviter de modifier directement l'entrée
  let triangles = crossedTriangles.map((triangle) => ({
    ...triangle,
    edgesTouched: [...triangle.edgesTouched], // Cloner edgesTouched
  }));

  let previousLength; // Stocker la longueur précédente pour comparaison

  while (true) {
    previousLength = triangles.length; // Longueur avant cette itération
    let redundantTriangleIndex = -1;
    let redundantEdge = null;

    // Étape 1 : Identifier un triangle redondant et l'arête correspondante
    for (let i = 0; i < triangles.length; i++) {
      const { edgesTouched } = triangles[i];

      if (edgesTouched.length === 0) {
        continue; // Passer les triangles sans arêtes touchées
      }
      ...
      if (stack.length === 0) {
        ...
        redundantEdge = edgesTouched[0]; // On peut choisir la première arête comme représentative
        break;
      }
    }

    if (redundantTriangleIndex === -1) {
      // Aucun triangle redondant trouvé
      break;
    }

    // Étape 2 : Supprimer le triangle redondant
    triangles.splice(redundantTriangleIndex, 1);

    // Étape 3 : Supprimer l'arête redondante des autres triangles
    triangles.forEach((triangle) => {
      triangle.edgesTouched = triangle.edgesTouched.filter(
        (edge) =>
          !(
            (edge[0].x === redundantEdge[0].x &&
              edge[0].y === redundantEdge[0].y &&
              edge[1].x === redundantEdge[1].x &&
              edge[1].y === redundantEdge[1].y) ||
            (edge[0].x === redundantEdge[1].x &&
              edge[0].y === redundantEdge[1].y &&
              edge[1].x === redundantEdge[0].x &&
              edge[1].y === redundantEdge[0].y)
          )
      );
    });
    console.log("Je passe");
    // Si la longueur des triangles n'a pas changé, on sort de la boucle
    if (triangles.length === previousLength) {
      break;
    }
  }

  console.log("Sortie finale :", triangles);
  return triangles;
}
function liftPolygon(tthr, crossedTriangles) {
...
function splitIntoContiguous(segments, edgesTouched) {
  ...
  for (let i = 1; i < segments.length; i++) {
    if (segments[i] === segments[i - 1] + 1) {
      ...
      if (currentGroup[0] === 0 && currentGroup.length === 1) {
